...
struct RunConfig {
    ...
    // Maximum bytes reserved by the look-ahead hot set. Outstanding late
    // requests remain reserved until the background prefetcher finishes them.
...
struct PrefetchStats {
    ...
    std::uint64_t max_pending_prefetch_bytes = 0;
    std::uint64_t max_prefetched_cache_bytes = 0;
