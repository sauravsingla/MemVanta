...
class PagedKVCache {
  public:
    explicit PagedKVCache(std::size_t kv_dim = 0,
                          std::size_t page_tokens = 128,
    ...
    void
    add_value(std::size_t pos, std::size_t offset, float alpha, float* out, std::size_t n) const;
  ...
  private:
    ...
    std::size_t kv_dim_{}, page_tokens_{};
    KVCacheType type_{KVCacheType::F32};
...
class LlamaModel {
  public:
    LlamaModel(const std::string& gguf_path,
               unsigned threads = 1,
               std::size_t ctx_override = 0,
               std::size_t kv_page_tokens = 128,
               KVCacheType kv_type = KVCacheType::F16);
    ...
    const std::vector<float>& prefill(const std::vector<int>& tokens,
                                      std::size_t batch_size = 16,
                                      bool compute_logits_last = true);
  ...
  private:
    ...
    std::vector<float> batch_x_, batch_n_, batch_q_, batch_k_, batch_v_, batch_att_, batch_proj_,
        batch_gate_, batch_up_, batch_ff_, batch_out_;
    ...
    forward_batch_impl(const int* tokens, std::size_t batch, bool compute_logits_last);
