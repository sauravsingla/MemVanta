...
void rope_one(std::vector<float>& z, std::size_t pos, std::size_t heads, float rope_theta) {
    ...
    for (std::size_t head = 0; head < heads; ++head) {
        ...
        for (std::size_t i = 0; i + 1 < hd; i += 2) {
            const float freq =
                std::pow(rope_theta, -static_cast<float>(i) / static_cast<float>(hd));
...
class MiniLlama {
  public:
    explicit MiniLlama(const Config& c) : c_(c), lm_(c.vocab, c.hidden, 0xABCDEF) {
        if (c.hidden % 32 || c.ff % 32 || c.hidden % c.heads || c.heads % c.kv_heads)
            throw std::runtime_error("invalid dimensions");
        const std::size_t kv_dim = (c.hidden / c.heads) * c.kv_heads;
        if (kv_dim % 32)
            throw std::runtime_error("kv dimension must be multiple of 32");
        layers_.reserve(c.layers);
        for (std::size_t i = 0; i < c.layers; ++i)
            layers_.emplace_back(c.hidden, kv_dim, c.ff, 0x10000 + i * 17);
    ...
    int step(int token, bool logits = true) {
        ...
        for (std::size_t li = 0; li < c_.layers; ++li) {
            ...
            std::memcpy(kc, k_.data(), kv_dim * sizeof(float));
            std::memcpy(vc, v_.data(), kv_dim * sizeof(float));
            ...
            for (std::size_t head = 0; head < c_.heads; ++head) {
                ...
                for (std::size_t t = 0; t <= pos_; ++t) {
                    const float* kt = &kcache_[(li * c_.max_ctx + t) * kv_dim + kv_hb];
                    float s = 0;
                ...
                for (std::size_t t = 0; t <= pos_; ++t) {
                    ...
                    const float* vt = &vcache_[(li * c_.max_ctx + t) * kv_dim + kv_hb];
                    for (std::size_t d = 0; d < hd; ++d)
                        att_[hb + d] += a * vt[d];
                }
            }
...
int main_impl(int argc, char** argv) {
    ...
    std::cout << "| model | size | params | backend | threads | test | result "
                 "|\n|---|---:|---:|---|---:|---|---:|\n";
    if (!skip_core) {
        std::cout << "| MemVanta SmolLM2-shaped Q8_0 | " << std::fixed << std::setprecision(2)
                  << mib << " MiB | " << m.params() / 1e6 << " M | CPU/AVX2 | " << c.threads
                  << " | pp512 | " << spp.mean << " ± " << spp.sd << " t/s |\n";
        std::cout << "| MemVanta SmolLM2-shaped Q8_0 | " << mib << " MiB | " << m.params() / 1e6
    ...
    if (!csv.empty()) {
        ...
        const std::size_t n = std::max(r.pp_tps.size(), r.ttft_ms.size());
        for (std::size_t i = 0; i < n; ++i) {
            f << i + 1 << ",";
            if (i < r.pp_tps.size())
                f << r.pp_tps[i];
