...
namespace {
std::uint64_t mib(const std::string& s){return std::stoull(s)<<20;}
}

int main(int argc,char**argv){
  try{
    if(argc<2){
      std::cerr<<"usage: memvanta_bench FILE [--mode none|fixed|adaptive] [--slice-mib N] [--cache-mib N] [--passes N] [--depth N] [--min-depth N] [--max-depth N] [--window N] [--budget-mib N]\n";
      return 1;
    }
    std::string path=argv[1],mode="adaptive";
    std::uint64_t slice_bytes=64ull<<20,cache_bytes=256ull<<20,budget_bytes=256ull<<20;
    ...
    for(int i=2;i<argc;++i){
      ...
      if(a=="--mode")mode=value();
      else if(a=="--slice-mib")slice_bytes=mib(value());
      else if(a=="--cache-mib")cache_bytes=mib(value());
      else if(a=="--budget-mib")budget_bytes=mib(value());
      else if(a=="--passes")passes=static_cast<std::uint32_t>(std::stoul(value()));
      else if(a=="--depth")depth=static_cast<std::uint32_t>(std::stoul(value()));
      else if(a=="--min-depth")min_depth=static_cast<std::uint32_t>(std::stoul(value()));
      else if(a=="--max-depth")max_depth=static_cast<std::uint32_t>(std::stoul(value()));
      else if(a=="--window")window=static_cast<std::uint32_t>(std::stoul(value()));
      else throw std::runtime_error("unknown argument: "+a);
    }
    if(mode!="none"&&mode!="fixed"&&mode!="adaptive")throw std::runtime_error("mode must be none, fixed, or adaptive");
    if(!slice_bytes||!cache_bytes||!passes)throw std::runtime_error("slice/cache/passes must be non-zero");
    ...
    std::cout<<std::fixed<<std::setprecision(6)
      <<"mode="<<mode
      <<" throughput_gib_s="<<s.gib_per_s
      <<" seconds="<<s.seconds
      <<" peak_rss_mib="<<(s.peak_rss_kb/1024.0)
      <<" checksum="<<s.checksum
      <<" eligible="<<s.prefetch.eligible
      <<" requests="<<s.prefetch.requests
      <<" useful="<<s.prefetch.useful
      <<" unused="<<s.prefetch.unused
      <<" late="<<s.prefetch.late
      ...
      <<"\n";
    if(s.prefetch.hot_set_budget_bytes&&s.prefetch.max_inflight_bytes>s.prefetch.hot_set_budget_bytes)
