...
std::uint64_t parse_size(const std::string& s) {
    ...
    std::transform(
        suf.begin(), suf.end(), suf.begin(), [](unsigned char c) { return std::tolower(c); });
    double m = 1;
    if (suf == "k" || suf == "kb" || suf == "kib")
        m = 1024.;
    else if (suf == "m" || suf == "mb" || suf == "mib")
        m = 1024. * 1024.;
    else if (suf == "g" || suf == "gb" || suf == "gib")
        m = 1024. * 1024. * 1024.;
    else if (!suf.empty() && suf != "b")
        throw std::runtime_error("bad size suffix: " + suf);
