...
RunStats Runtime::run_stream() {
  ...
  AdaptivePrefetchController controller(
      {cfg_.adaptive_min_depth, cfg_.adaptive_max_depth, cfg_.adaptive_low_useful_ratio,
       cfg_.adaptive_high_useful_ratio},
      depth);
    ...
    for (std::uint32_t i = 0; i < store_.count(); ++i) {
      ...
      if (cfg_.adaptive_prefetch && window_items >= cfg_.adaptive_window) {
        ...
        const auto decision = controller.observe(
            {average_item_ms, window_consumed, window_useful, window_late,
             window_budget_skips, cache_.stats().evictions, window_peak_inflight,
             inflight_limit});
