...
void WorkerPool::worker(unsigned tid) {
    ...
    for (;;) {
        ...
        const std::size_t step = (n + threads_ - 1) / threads_, a = tid * step,
                          b = std::min(n, a + step);
        try {
            if (a < b)
                f(a, b);
            ...
            std::lock_guard<std::mutex> lk(m_);
            if (g == generation_ && !error_)
                error_ = std::current_exception();
        }
        {
            std::lock_guard<std::mutex> lk(m_);
            if (g == generation_ && ++finished_ == threads_ - 1)
                done_.notify_one();
        }
    }
}
void WorkerPool::parallel_for(std::size_t n,
    ...
    std::unique_lock<std::mutex> lk(m_);
    done_.wait(lk, [&] { return finished_ == threads_ - 1; });
    auto error = error_;
    lk.unlock();
    if (error)
