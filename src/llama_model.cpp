...
const std::vector<float>& LlamaModel::forward(int token,bool compute_logits){
    ...
    for(std::size_t li=0;li<c_.n_layer;++li){auto&L=layers_[li];rmsnorm(x_,L.attn_norm_f,n_);MatvecTarget qkv[]={{L.q,q_.data()},{L.k,k_.data()},{L.v,v_.data()}};tensor_matvec_group(file_,qkv,3,n_.data(),threads_,nullptr);rope(q_,k_,pos_);kv_[li].write(pos_,k_.data(),v_.data());std::fill(att_.begin(),att_.end(),0.0f);
        ...
        tensor_matvec(file_,*L.o,att_.data(),proj_.data(),threads_,nullptr);for(std::size_t i=0;i<c_.n_embd;++i)x_[i]+=proj_[i];rmsnorm(x_,L.ffn_norm_f,n_);MatvecTarget ffn_in[]={{L.gate,gate_.data()},{L.up,up_.data()}};tensor_matvec_group(file_,ffn_in,2,n_.data(),threads_,nullptr);for(std::size_t i=0;i<c_.n_ff;++i){float z=gate_[i];ff_[i]=(z/(1.0f+std::exp(-z)))*up_[i];}tensor_matvec(file_,*L.down,ff_.data(),out_.data(),threads_,nullptr);for(std::size_t i=0;i<c_.n_embd;++i)x_[i]+=out_[i];}
