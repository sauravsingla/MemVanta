...
std::size_t workspace_limit_bytes(){
    ...
    if(!raw||!*raw)return default_limit;
    try{
        std::size_t used=0;const auto v=std::stoull(raw,&used,10);
        if(used!=std::strlen(raw)||v>std::numeric_limits<std::size_t>::max())throw std::runtime_error("invalid MEMVANTA_MAX_WORKSPACE_BYTES");
        return static_cast<std::size_t>(v);
    }catch(const std::exception&){throw std::runtime_error("invalid MEMVANTA_MAX_WORKSPACE_BYTES");}
}

int optional_token_id(const GgufFile& f,const char* key){
    const auto v=f.get_u64(key);if(!v||*v>static_cast<std::uint64_t>(std::numeric_limits<int>::max()))return -1;return static_cast<int>(*v);
}
...
PagedKVCache::Page& PagedKVCache::ensure(std::size_t p){
    if(p>=pages_.size())pages_.resize(checked_add_size(p,1,"KV page index overflow"));
    if(!pages_[p])pages_[p]=std::make_unique<Page>(checked_mul_size(page_tokens_,kv_dim_,"KV page element count overflow"),page_tokens_,type_);
    return *pages_[p];
}
void PagedKVCache::write(std::size_t pos,const float*k,const float*v){
    if(kv_dim_&&(!k||!v))throw std::runtime_error("null KV cache input");
    auto p=pos/page_tokens_,i=pos%page_tokens_;auto&pg=ensure(p);auto off=checked_mul_size(i,kv_dim_,"KV cache offset overflow");
    if(type_==KVCacheType::F32){std::memcpy(pg.k32.data()+off,k,checked_mul_size(kv_dim_,sizeof(float),"KV copy size overflow"));std::memcpy(pg.v32.data()+off,v,checked_mul_size(kv_dim_,sizeof(float),"KV copy size overflow"));}
    else if(type_==KVCacheType::F16){for(std::size_t d=0;d<kv_dim_;++d){pg.k16[off+d]=fp32_to_fp16(k[d]);pg.v16[off+d]=fp32_to_fp16(v[d]);}}
    else{float km=0,vm=0;for(std::size_t d=0;d<kv_dim_;++d){km=std::max(km,std::abs(k[d]));vm=std::max(vm,std::abs(v[d]));}float ks=km>0?km/127.0f:1.0f,vs=vm>0?vm/127.0f:1.0f;pg.ks[i]=ks;pg.vs[i]=vs;for(std::size_t d=0;d<kv_dim_;++d){pg.k8[off+d]=static_cast<std::int8_t>(std::clamp(std::lround(k[d]/ks),-127l,127l));pg.v8[off+d]=static_cast<std::int8_t>(std::clamp(std::lround(v[d]/vs),-127l,127l));}}
}
float PagedKVCache::dot_key(std::size_t pos,std::size_t offset,const float*q,std::size_t n)const{
    if(offset>kv_dim_||n>kv_dim_-offset)throw std::runtime_error("KV key slice out of bounds");if(n&&!q)throw std::runtime_error("null KV query");
    auto p=pos/page_tokens_,i=pos%page_tokens_;if(p>=pages_.size()||!pages_[p])throw std::runtime_error("KV key page missing");auto&pg=*pages_[p];auto off=checked_add_size(checked_mul_size(i,kv_dim_,"KV key offset overflow"),offset,"KV key offset overflow");
    if(type_==KVCacheType::F32)return dot_f32_simd(q,pg.k32.data()+off,n);
    if(type_==KVCacheType::F16){
#if defined(__AVX2__) && defined(__F16C__)
        __m256 acc=_mm256_setzero_ps();std::size_t d=0;for(;d+8<=n;d+=8){__m128i h=_mm_loadu_si128(reinterpret_cast<const __m128i*>(pg.k16.data()+off+d));__m256 vv=_mm256_cvtph_ps(h);acc=_mm256_fmadd_ps(_mm256_loadu_ps(q+d),vv,acc);}alignas(32)float tmp[8];_mm256_store_ps(tmp,acc);float sum=tmp[0]+tmp[1]+tmp[2]+tmp[3]+tmp[4]+tmp[5]+tmp[6]+tmp[7];for(;d<n;++d)sum+=q[d]*fp16_to_fp32(pg.k16[off+d]);return sum;
#else
...
void PagedKVCache::add_value(std::size_t pos,std::size_t offset,float alpha,float*out,std::size_t n)const{
    if(offset>kv_dim_||n>kv_dim_-offset)throw std::runtime_error("KV value slice out of bounds");if(n&&!out)throw std::runtime_error("null KV output");
    auto p=pos/page_tokens_,i=pos%page_tokens_;if(p>=pages_.size()||!pages_[p])throw std::runtime_error("KV value page missing");auto&pg=*pages_[p];auto off=checked_add_size(checked_mul_size(i,kv_dim_,"KV value offset overflow"),offset,"KV value offset overflow");if(type_==KVCacheType::F32){axpy_f32_simd(alpha,pg.v32.data()+off,out,n);return;}
    if(type_==KVCacheType::F16){
#if defined(__AVX2__) && defined(__F16C__)
        __m256 av=_mm256_set1_ps(alpha);std::size_t d=0;for(;d+8<=n;d+=8){__m128i h=_mm_loadu_si128(reinterpret_cast<const __m128i*>(pg.v16.data()+off+d));__m256 vv=_mm256_cvtph_ps(h);__m256 y=_mm256_loadu_ps(out+d);_mm256_storeu_ps(out+d,_mm256_fmadd_ps(av,vv,y));}for(;d<n;++d)out[d]+=alpha*fp16_to_fp32(pg.v16[off+d]);return;
#else
...
std::size_t PagedKVCache::bytes_allocated()const{
    std::size_t n=0;auto add=[&](std::size_t count,std::size_t width){n=checked_add_size(n,checked_mul_size(count,width,"KV byte accounting overflow"),"KV byte accounting overflow");};
    for(auto&p:pages_)if(p){add(p->k32.size(),4);add(p->v32.size(),4);add(p->k16.size(),2);add(p->v16.size(),2);add(p->k8.size(),1);add(p->v8.size(),1);add(p->ks.size(),4);add(p->vs.size(),4);}return n;
}
    ...
    while(p<text.size()){
        ...
        if(u_space(u.cp)){
            if(u.cp==0x20 && p+1<text.size()){
                ...
                if(!u_space(nx.cp)){
                    std::size_t qpos=p+1; enum{L,D,P} kind=u_digit(nx.cp)?D:(u_letter(nx.cp)?L:P);
                    while(qpos<text.size()){auto xx=utf8_at(text,qpos);auto kk=u_digit(xx.cp)?D:(u_letter(xx.cp)?L:P);if(u_space(xx.cp)||kk!=kind)break;qpos+=xx.len;}
                    out.push_back(text.substr(p,qpos-p));p=qpos;continue;
                }
            }
            std::size_t qpos=p+u.len;while(qpos<text.size()){auto xx=utf8_at(text,qpos);if(!u_space(xx.cp))break;qpos+=xx.len;}out.push_back(text.substr(p,qpos-p));p=qpos;continue;
        }
        enum{L,D,P} kind=u_digit(u.cp)?D:(u_letter(u.cp)?L:P);std::size_t qpos=p+u.len;
        while(qpos<text.size()){std::size_t xcn=0;if(starts_contraction(qpos,xcn))break;auto xx=utf8_at(text,qpos);auto kk=u_digit(xx.cp)?D:(u_letter(xx.cp)?L:P);if(u_space(xx.cp)||kk!=kind)break;qpos+=xx.len;}
        out.push_back(text.substr(p,qpos-p));p=qpos;
    }
    return out;
}
...
std::vector<int> GgufTokenizer::encode_gpt2(const std::string& text) const{
    ...
    auto bpe_one=[&](const std::string& raw){
        ...
        for(const auto&z:syms){auto it=token_to_id_.find(z);if(it!=token_to_id_.end()){ids.push_back(it->second);continue;}for(std::size_t i=0;i<z.size();){auto u=utf8_at(z,i);auto atom=z.substr(i,u.len);auto jt=token_to_id_.find(atom);if(jt==token_to_id_.end()){if(unk_>=0)ids.push_back(unk_);else throw std::runtime_error("GPT-2 byte token absent from GGUF vocabulary");}else ids.push_back(jt->second);i+=u.len;}}
    };
    std::size_t p=0;while(p<text.size()){
        ...
        std::size_t qpos=p+1;while(qpos<text.size()){bool hit=false;for(const auto&sp:specials_){const auto&t=sp.first;if(!t.empty()&&t.size()<=text.size()-qpos&&text.compare(qpos,t.size(),t)==0){hit=true;break;}}if(hit)break;++qpos;}
        auto chunks=pretokenize_gpt2(text.substr(p,qpos-p));for(auto&c:chunks)bpe_one(c);p=qpos;
    }
    return ids;
}
...
void LlamaModel::load_config(std::size_t ctx_override,std::size_t kv_page_tokens){
    ...
    auto req=[&](const char*k){auto v=file_.get_u64(k);if(!v)throw std::runtime_error(std::string("missing GGUF metadata: ")+k);if(*v>std::numeric_limits<std::size_t>::max())throw std::runtime_error(std::string("GGUF metadata exceeds address space: ")+k);return static_cast<std::size_t>(*v);};
    c_.n_ctx=req("llama.context_length");c_.n_embd=req("llama.embedding_length");c_.n_ff=req("llama.feed_forward_length");c_.n_layer=req("llama.block_count");c_.n_head=req("llama.attention.head_count");
    if(auto hv=file_.get_u64("llama.attention.head_count_kv")){if(*hv>std::numeric_limits<std::size_t>::max())throw std::runtime_error("GGUF KV head count exceeds address space");c_.n_head_kv=static_cast<std::size_t>(*hv);}else c_.n_head_kv=c_.n_head;
    c_.rms_eps=static_cast<float>(file_.get_f64("llama.attention.layer_norm_rms_epsilon").value_or(1e-5));c_.rope_theta=static_cast<float>(file_.get_f64("llama.rope.freq_base").value_or(10000.0));
    ...
    if(c_.n_layer>file_.tensors().size()/9)throw std::runtime_error("Llama layer count is inconsistent with GGUF tensor count");
    ...
    for(std::size_t i=0;i<c_.n_layer;++i){auto&L=layers_[i];auto name=[&](const char*s){return "blk."+std::to_string(i)+s;};L.attn_norm=&file_.tensor(name(".attn_norm.weight"));L.q=&file_.tensor(name(".attn_q.weight"));L.k=&file_.tensor(name(".attn_k.weight"));L.v=&file_.tensor(name(".attn_v.weight"));L.o=&file_.tensor(name(".attn_output.weight"));L.ffn_norm=&file_.tensor(name(".ffn_norm.weight"));L.gate=&file_.tensor(name(".ffn_gate.weight"));L.up=&file_.tensor(name(".ffn_up.weight"));L.down=&file_.tensor(name(".ffn_down.weight"));expect_matrix(*L.q,c_.n_embd,c_.n_embd);expect_matrix(*L.k,c_.n_embd,K);expect_matrix(*L.v,c_.n_embd,K);expect_matrix(*L.o,c_.n_embd,c_.n_embd);expect_matrix(*L.gate,c_.n_embd,c_.n_ff);expect_matrix(*L.up,c_.n_embd,c_.n_ff);expect_matrix(*L.down,c_.n_ff,c_.n_embd);L.attn_norm_f.resize(c_.n_embd);L.ffn_norm_f.resize(c_.n_embd);tensor_vector_to_f32(file_,*L.attn_norm,L.attn_norm_f.data(),c_.n_embd);tensor_vector_to_f32(file_,*L.ffn_norm,L.ffn_norm_f.data(),c_.n_embd);kv_.emplace_back(K,kv_page_tokens,kv_type_);}
    ...
    rope_cos_.clear();rope_sin_.clear();rope_cached_positions_=0;
    reset();
}
...
void LlamaModel::ensure_rope(std::size_t last_pos){
    if(last_pos>=c_.n_ctx)throw std::runtime_error("RoPE position exceeds context");if(last_pos<rope_cached_positions_)return;
    const std::size_t half=c_.head_dim()/2,new_positions=checked_add_size(last_pos,1,"RoPE position overflow"),new_elems=checked_mul_size(new_positions,half,"RoPE table size overflow"),old=rope_cached_positions_;
    rope_cos_.resize(new_elems);rope_sin_.resize(new_elems);
    for(std::size_t p=old;p<new_positions;++p)for(std::size_t i=0;i<half;++i){float inv=std::pow(c_.rope_theta,-2.0f*float(i)/float(c_.head_dim()));float a=float(p)*inv;rope_cos_[p*half+i]=std::cos(a);rope_sin_[p*half+i]=std::sin(a);}rope_cached_positions_=new_positions;
}
void LlamaModel::rope_ptr(float*q,float*k,std::size_t pos)const{if(pos>=rope_cached_positions_)throw std::runtime_error("RoPE position not prepared");const std::size_t hd=c_.head_dim(),half=hd/2;const float*co=rope_cos_.data()+pos*half,*si=rope_sin_.data()+pos*half;auto rot=[&](float*z,std::size_t heads){for(std::size_t h=0;h<heads;++h){float*b=z+h*hd;for(std::size_t i=0;i<half;++i){float x0=b[i],x1=b[i+half],cc=co[i],ss=si[i];b[i]=x0*cc-x1*ss;b[i+half]=x0*ss+x1*cc;}}};rot(q,c_.n_head);rot(k,c_.n_head_kv);}
...
const std::vector<float>& LlamaModel::forward_batch_impl(const int*tokens,std::size_t B,bool compute_logits_last){
    if(!B)return logits_;if(!tokens)throw std::runtime_error("null token batch");if(pos_>c_.n_ctx||B>c_.n_ctx-pos_)throw std::runtime_error("context exhausted");for(std::size_t b=0;b<B;++b)if(tokens[b]<0||static_cast<std::size_t>(tokens[b])>=c_.vocab)throw std::runtime_error("token id out of range");
    const std::size_t D=c_.n_embd,F=c_.n_ff,K=c_.kv_dim(),hd=c_.head_dim(),group=c_.n_head/c_.n_head_kv,start=pos_;
    const auto bd=checked_mul_size(B,D,"prefill D workspace overflow"),bk=checked_mul_size(B,K,"prefill KV workspace overflow"),bf=checked_mul_size(B,F,"prefill FF workspace overflow");
    std::size_t total=0;for(int i=0;i<6;++i)total=checked_add_size(total,bd,"prefill workspace overflow");for(int i=0;i<2;++i)total=checked_add_size(total,bk,"prefill workspace overflow");for(int i=0;i<3;++i)total=checked_add_size(total,bf,"prefill workspace overflow");
    const auto bytes=checked_mul_size(total,sizeof(float),"prefill workspace byte-size overflow");if(bytes>workspace_limit_bytes())throw std::runtime_error("prefill workspace exceeds MEMVANTA_MAX_WORKSPACE_BYTES");
    batch_x_.resize(bd);batch_n_.resize(bd);batch_q_.resize(bd);batch_k_.resize(bk);batch_v_.resize(bk);batch_att_.resize(bd);batch_proj_.resize(bd);batch_gate_.resize(bf);batch_up_.resize(bf);batch_ff_.resize(bf);batch_out_.resize(bd);
    auto&X=batch_x_;auto&N=batch_n_;auto&Q=batch_q_;auto&KK=batch_k_;auto&VV=batch_v_;auto&ATT=batch_att_;auto&PROJ=batch_proj_;auto&G=batch_gate_;auto&U=batch_up_;auto&FF=batch_ff_;auto&OUT=batch_out_;
    ...
    for(std::size_t li=0;li<c_.n_layer;++li){auto&L=layers_[li];rms_batch(X,L.attn_norm_f,N);tensor_matmul_batch_v06(file_,*L.q,N.data(),Q.data(),B,threads_,&pool_);tensor_matmul_batch_v06(file_,*L.k,N.data(),KK.data(),B,threads_,&pool_);tensor_matmul_batch_v06(file_,*L.v,N.data(),VV.data(),B,threads_,&pool_);
        ...
        std::fill(ATT.begin(),ATT.end(),0.0f);pool_.parallel_for(checked_mul_size(B,c_.n_head,"attention job count overflow"),[&](std::size_t a,std::size_t z){std::vector<float> scores;for(std::size_t job=a;job<z;++job){std::size_t b=job/c_.n_head,h=job%c_.n_head,kh=h/group,qb=h*hd,kb=kh*hd,upto=start+b;scores.resize(upto+1);float mx=-std::numeric_limits<float>::infinity();for(std::size_t t=0;t<=upto;++t){float sf=kv_[li].dot_key(t,kb,Q.data()+b*D+qb,hd)/std::sqrt(float(hd));scores[t]=sf;mx=std::max(mx,sf);}double den=0;for(float&vv:scores){vv=std::exp(vv-mx);den+=vv;}float inv=float(1.0/den);float*out=ATT.data()+b*D+qb;for(std::size_t t=0;t<=upto;++t)kv_[li].add_value(t,kb,scores[t]*inv,out,hd);}});
        tensor_matmul_batch_v06(file_,*L.o,ATT.data(),PROJ.data(),B,threads_,&pool_);pool_.parallel_for(bd,[&](std::size_t a,std::size_t z){for(std::size_t i=a;i<z;++i)X[i]+=PROJ[i];});rms_batch(X,L.ffn_norm_f,N);tensor_matmul_batch_v06(file_,*L.gate,N.data(),G.data(),B,threads_,&pool_);tensor_matmul_batch_v06(file_,*L.up,N.data(),U.data(),B,threads_,&pool_);pool_.parallel_for(bf,[&](std::size_t a,std::size_t z){for(std::size_t i=a;i<z;++i){float xx=G[i];FF[i]=(xx/(1.0f+std::exp(-xx)))*U[i];}});tensor_matmul_batch_v06(file_,*L.down,FF.data(),OUT.data(),B,threads_,&pool_);pool_.parallel_for(bd,[&](std::size_t a,std::size_t z){for(std::size_t i=a;i<z;++i)X[i]+=OUT[i];});
    }
...
const std::vector<float>& LlamaModel::forward(int token,bool compute_logits){
    if(token<0||static_cast<std::size_t>(token)>=c_.vocab)throw std::runtime_error("token id out of range");if(pos_>=c_.n_ctx)throw std::runtime_error("context exhausted");ensure_rope(pos_);tensor_read_row_f32(file_,*token_embd_,static_cast<std::size_t>(token),x_.data(),c_.n_embd);const std::size_t hd=c_.head_dim(),group=c_.n_head/c_.n_head_kv;
    for(std::size_t li=0;li<c_.n_layer;++li){auto&L=layers_[li];rmsnorm(x_,L.attn_norm_f,n_);MatvecTarget qkv[]={{L.q,q_.data()},{L.k,k_.data()},{L.v,v_.data()}};tensor_matvec_group(file_,qkv,3,n_.data(),threads_,&pool_);rope(q_,k_,pos_);kv_[li].write(pos_,k_.data(),v_.data());std::fill(att_.begin(),att_.end(),0.0f);
        ...
        tensor_matvec(file_,*L.o,att_.data(),proj_.data(),threads_,&pool_);for(std::size_t i=0;i<c_.n_embd;++i)x_[i]+=proj_[i];rmsnorm(x_,L.ffn_norm_f,n_);MatvecTarget ffn_in[]={{L.gate,gate_.data()},{L.up,up_.data()}};tensor_matvec_group(file_,ffn_in,2,n_.data(),threads_,&pool_);for(std::size_t i=0;i<c_.n_ff;++i){float z=gate_[i];ff_[i]=(z/(1.0f+std::exp(-z)))*up_[i];}tensor_matvec(file_,*L.down,ff_.data(),out_.data(),threads_,&pool_);for(std::size_t i=0;i<c_.n_embd;++i)x_[i]+=out_[i];}
    ++pos_;if(compute_logits){rmsnorm(x_,output_norm_f_,n_);tensor_matvec(file_,*output_,n_.data(),logits_.data(),threads_,&pool_);}return logits_;
}
...
std::size_t LlamaModel::kv_bytes_allocated()const{std::size_t n=0;for(auto&k:kv_)n=checked_add_size(n,k.bytes_allocated(),"KV byte accounting overflow");return n;}
std::size_t LlamaModel::rope_bytes_allocated()const{return checked_mul_size(checked_add_size(rope_cos_.size(),rope_sin_.size(),"RoPE byte accounting overflow"),sizeof(float),"RoPE byte accounting overflow");}
std::size_t LlamaModel::scratch_bytes_allocated()const{
    std::size_t n=0;auto add=[&](const std::vector<float>&v){n=checked_add_size(n,checked_mul_size(v.capacity(),sizeof(float),"scratch byte accounting overflow"),"scratch byte accounting overflow");};
    add(batch_x_);add(batch_n_);add(batch_q_);add(batch_k_);add(batch_v_);add(batch_att_);add(batch_proj_);add(batch_gate_);add(batch_up_);add(batch_ff_);add(batch_out_);return n;
}
