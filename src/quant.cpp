...
std::vector<BlockQ8_0> quantize_q8_0(const float* src, std::size_t n) {
    if (n % QK)
        throw std::runtime_error("q8_0 length must be multiple of 32");
    ...
    for (std::size_t b = 0; b < out.size(); ++b) {
        ...
        out[b].d = d;
        if (d == 0.0f) {
            std::fill(std::begin(out[b].qs), std::end(out[b].qs), 0);
            continue;
        }
...
std::vector<BlockQ4_0> quantize_q4_0(const float* src, std::size_t n) {
    if (n % QK)
        throw std::runtime_error("q4_0 length must be multiple of 32");
    ...
    for (std::size_t b = 0; b < out.size(); ++b) {
        ...
        for (std::size_t i = 0; i < QK; ++i) {
            ...
            if ((i & 1u) == 0)
                ...
                out[b].qs[i / 2] |= static_cast<std::uint8_t>(nib << 4);
        }
    }
    return out;
}
...
float dot_q8_0(const BlockQ8_0* a, const float* x, std::size_t n) {
    ...
    for (std::size_t b = 0; b < nb; ++b) {
        ...
        for (std::size_t k = 0; k < QK; ++k)
            s += static_cast<float>(a[b].qs[k]) * x[b * QK + k];
        sum += s * a[b].d;
#endif
    }
    return sum;
...
void matvec_q8_0(const BlockQ8_0* A,
                 ...
                 unsigned threads) {
    if (cols % QK)
        throw std::runtime_error("q8_0 cols must be multiple of 32");
