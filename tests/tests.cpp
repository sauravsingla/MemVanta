...
int main() {
...

  {
    ...
    memvanta::TensorCache c(128);
    auto a = c.get_or_load(1, src.data(), 64);
    auto b = c.get_or_load(1, src.data(), 64);
    const auto st = c.stats();
    CHECK(st.hits == 1 && st.misses == 1);
    (void)a;
    (void)b;
  }

  {
    std::vector<std::byte> src(64);
    memvanta::TensorCache c(128);
    c.insert_prefetched(7, src.data(), 64);
    CHECK(c.prefetched_bytes() == 64);
    CHECK(c.peak_prefetched_bytes() == 64);
    auto hit = c.get_or_load(7, src.data(), 64);
    CHECK(c.prefetched_bytes() == 0);
    CHECK(c.peak_prefetched_bytes() == 64);
    (void)hit;
  }

  {
    bool threw = false;
    try {
      (void)memvanta::ggml_tensor_nbytes(memvanta::GgmlType::F32,
...

  {
    ...
    bool parsed = false;
    try {
      memvanta::GgufFile g(path);
      parsed = g.tensors().empty();
    } catch (...) {
      parsed = false;
    }
    CHECK_MSG(parsed, "metadata-only GGUF with zero tensors was rejected");
    std::remove(path);
    ...
    bool threw = false;
    try {
      memvanta::GgufFile g(truncated);
      (void)g;
    } catch (const std::runtime_error&) {
      threw = true;
    }
    CHECK_MSG(threw, "truncated GGUF header was not rejected");
...

  {
    const char* path = "memvanta_bad_quant_row_test.gguf";
    {
      ...
      const std::string name = "bad.weight";
      std::uint64_t name_len = name.size();
      std::uint32_t nd = 2;
      std::uint64_t d0 = 16;
      std::uint64_t d1 = 2;
      std::uint32_t type = static_cast<std::uint32_t>(memvanta::GgmlType::Q4_0);
      std::uint64_t rel = 0;
      ...
      f.write(reinterpret_cast<const char*>(&d0), sizeof(d0));
      f.write(reinterpret_cast<const char*>(&d1), sizeof(d1));
      f.write(reinterpret_cast<const char*>(&type), sizeof(type));
      f.write(reinterpret_cast<const char*>(&rel), sizeof(rel));
      const auto pos = static_cast<std::uint64_t>(f.tellp());
      const auto aligned = (pos + 31u) & ~std::uint64_t(31u);
      for (std::uint64_t i = pos; i < aligned + 18; ++i) {
        f.put('\0');
      }
    }
    bool threw = false;
    ...
    CHECK_MSG(threw, "quantized tensor with a non-block-aligned row was not rejected");
    std::remove(path);
  }

  {
    ...
    const auto q8 = memvanta::quantize_q8_0(a.data(), n);
    const auto q4 = memvanta::quantize_q4_0(a.data(), n);
    const float e8 =
        std::fabs(memvanta::dot_q8_0(q8.data(), x.data(), n) - ref) / (std::fabs(ref) + 1e-6f);
    const float e4 =
        std::fabs(memvanta::dot_q4_0(q4.data(), x.data(), n) - ref) / (std::fabs(ref) + 1e-6f);
...

  {
    ...
    pool.parallel_for(seen.size(), [&](std::size_t a, std::size_t b) {
    ...
    bool threw = false;
    try {
      pool.parallel_for(64, [&](std::size_t a, std::size_t) {
        if (a > 0) {
    ...
    CHECK_MSG(std::count(seen.begin(), seen.end(), 0) == 0,
              "WorkerPool did not recover after a propagated exception");
  }

  {
    constexpr std::size_t d = 64;
    std::vector<float> k(d), v(d), q(d), out(d), refv(d);
    std::mt19937 g(11);
    ...
    for (auto typ : {memvanta::KVCacheType::F32, memvanta::KVCacheType::F16,
                     memvanta::KVCacheType::Q8}) {
      memvanta::PagedKVCache c(d, 16, typ);
      c.write(0, k.data(), v.data());
      std::fill(out.begin(), out.end(), 0);
      const float got = c.dot_key(0, 0, q.data(), d);
      c.add_value(0, 0, 0.25f, out.data(), d);
      float de = std::fabs(got - refdot) / (std::fabs(refdot) + 1e-5f);
      float ve = 0;
      float base = 0;
      for (std::size_t i = 0; i < d; ++i) {
        ve += std::fabs(out[i] - refv[i]);
        base += std::fabs(refv[i]);
      }
      ve /= base + 1e-5f;
      const char* who = memvanta::kv_cache_type_name(typ);
      if (typ == memvanta::KVCacheType::F32) {
        CHECK_MSG(de < 1e-5f, who);
        CHECK_MSG(ve < 1e-5f, who);
      } else if (typ == memvanta::KVCacheType::F16) {
        CHECK_MSG(de < 0.01f, who);
        CHECK_MSG(ve < 0.01f, who);
      } else {
        CHECK_MSG(de < 0.08f, who);
        CHECK_MSG(ve < 0.03f, who);
      }
    }
  }

  {
    const char* path = "memvanta_adaptive_prefetch_test.bin";
    {
      ...
      for (int i = 0; i < 32768; ++i) {
        const unsigned char b = static_cast<unsigned char>(i * 17);
        f.write(reinterpret_cast<const char*>(&b), 1);
      }
    }
    memvanta::TensorStore store(path, 4096);
    memvanta::RunConfig fixed{16384, 2, 2, true};
    const auto a = memvanta::Runtime(store, fixed).run_stream();
    memvanta::RunConfig adaptive{16384, 2, 2, true};
    adaptive.adaptive_prefetch = true;
    adaptive.adaptive_min_depth = 1;
    adaptive.adaptive_max_depth = 3;
    adaptive.adaptive_window = 2;
    const auto b = memvanta::Runtime(store, adaptive).run_stream();
    ...
    CHECK(b.prefetch.max_pending_prefetch_bytes <= b.prefetch.hot_set_budget_bytes);
    CHECK(b.prefetch.max_prefetched_cache_bytes <= adaptive.cache_bytes);
...

  {
    ...
    const char* path = "memvanta_prefetch_late_accounting_test.bin";
    {
      std::ofstream f(path, std::ios::binary);
      for (int i = 0; i < 32768; ++i) {
        const unsigned char b = static_cast<unsigned char>(i * 31);
        f.write(reinterpret_cast<const char*>(&b), 1);
      }
    }

    memvanta::TensorStore store(path, 1);
    memvanta::RunConfig cfg{64, 1, 32, true};
    cfg.prefetch_budget_bytes = 16;
    ...
    bool second_run_threw = false;
    try {
      (void)runtime.run_stream();
    } catch (const std::runtime_error&) {
      second_run_threw = true;
    }
    CHECK_MSG(second_run_threw, "Runtime silently allowed a second run after stopping prefetch");
    std::remove(path);
  }
