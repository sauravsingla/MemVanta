...
int main(int argc,char**argv){
    ...
    CHECK_FATAL(g.model_type()=="gpt2");CHECK_FATAL(sp.model_type()=="llama");
    ...
    auto hi=g.encode("hello world",false);std::vector<int> hi_ref={262,267};CHECK_MSG(hi==hi_ref,"GPT-2 merge table did not produce the expected ids for \"hello world\"");
    auto special=g.encode("<|im_start|>user\nhello<|im_end|>",false);CHECK_MSG(!special.empty()&&special.front()==1&&special.back()==2,"GGUF special tokens were not preserved verbatim");
    ...
    std::vector<C> cs={
      ...
      {"Café naïve résumé",{295,262,267,276,259,265,262,313,293,354,276,264,297,276}},
      {"emoji 😀 🚀 ❤️",{259,260,279,261,283,270,259,342,259,343,259,334,341}},
      {"  spaces   and\ttabs",{259,259,308,275,298,312,259,259,353,265,268,12,306,282,264}},
    ...
    for(const auto&c:cs){auto got=sp.encode(c.s,false);auto msg="Llama/GGUF tokenizer parity mismatch: "+c.s+" got="+ids_text(got)+" expected="+ids_text(c.ids);CHECK_MSG(got==c.ids,msg.c_str());roundtrip(sp,c.s);}
    CHECK(sp.encode("",false).empty());CHECK(g.encode("",false).empty());
    if(memvanta_test::failures()){ std::cerr<<"FAILED\n"; return 1; }
    std::cout<<"tokenizer tests ok: 2 tokenizer families, 202 byte-fuzz cases, 6 exact Llama/GGUF SPM vectors\n";
    return 0;
}
