...
int main() {
    phase("worker-pool-reuse");
    {
        ...
        for (int iter = 0; iter < 1000; ++iter) {
            ...
            CHECK_MSG(std::count(seen.begin(), seen.end(), iter + 1) ==
                          static_cast<std::ptrdiff_t>(seen.size()),
                      "WorkerPool stress run left indices unvisited");
        }
    }
    ...
    phase("worker-pool-exception-recovery");
    {
        ...
        for (int iter = 0; iter < 250; ++iter) {
            bool threw = false;
            try {
                pool.parallel_for(128, [&](std::size_t a, std::size_t) {
                    if (a > 0)
            ...
            CHECK_MSG(std::count(seen.begin(), seen.end(), 1) ==
                          static_cast<std::ptrdiff_t>(seen.size()),
                      "WorkerPool did not recover after stress exception");
        }
    }
    ...
    const char* path = "memvanta_concurrency_stress.bin";
    {
        ...
        for (int i = 0; i < 65536; ++i) {
            const unsigned char b = static_cast<unsigned char>((i * 37) ^ 0x5a);
    ...
    std::uint64_t reference = 0;
    {
        ...
        for (int iter = 0; iter < 64; ++iter) {
            ...
            CHECK_MSG(stats.checksum == reference,
                      "runtime/prefetch stress changed deterministic checksum");
        }
    }
    std::remove(path);
